...
def emulator_nfa(input_string, sectiuni):
    ...
    for c in input_string.strip():
        if c in sigma:
            next_states = set()  # colecteaza toate starile urmatoare pentru acest input
            for state in current_states:  # pentru fiecare stare curenta
                for a in transitions:  # cautam actiunile care pornesc din aceasta stare
                    if a.startswith(state + ',' + c + ','):  # cautam actiunea care arata sub forma 'current_state,digit in input,next_state'
                        next_state = a.split(',')[2]
                        if next_state not in next_states:
                            next_states.add(next_state)
            current_states = next_states  # setam starile urmatoare ca fiind starile curente
        else:
            pass
    condition = False
    for state in current_states:
        for c in states:
            if 'f' in c and c.split(',')[0] == state:
                condition = True
    if condition == True:  # cel putin o stare curenta e stare finala
        return 'Accepted'
    else:
        return 'Rejected'
...
if valid_file_name == True:
    ...
    while True:
        ...
        if verification_input(input_string):
            print(emulator_nfa(input_string, sectiuni))
        else:
            print("Input invalid!")
else:
